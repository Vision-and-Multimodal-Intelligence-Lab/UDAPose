...
from .transformer_deformable import (
...
class Transformer(nn.Module):
    def __init__(
        ...
        deformable_encoder=False,  # True
        deformable_decoder=False,  # True
    ...
    def forward(
        ...
        refpoint_embed,  # [bs, 100, 4], dn_bbox inverse_sigmoid
        input_query_keypoints,  # [bs, 100, 14 * 2], dn_keypoints inverse_sigmoid
        pos_embeds,  # len(4), poss[0].shape=([bs, 256, 98, 154]), 4-layer pos_embed
        tgt,  # [bs, 100, 256]
        attn_mask=None,  # attn_mask for stage 1 (box decoding)  # (64, 1000, 1000)  64 => bs*n_heads
        attn_mask2=None,  # attn_mask for stage 2 (keypoint decoding)  # (64, 1600, 1600)
    ):
        ...
        if self.two_stage_type in ["standard", "combine", "enceachlayer", "enclayer1"]:
            ...
            refpoint_embed_undetach = (
                self.enc_out_bbox_embed(enc_intermediate_output[-1]) + enc_refpoint_embed
            )  # unsigmoid, (bs, nq, 4)
        ...
        hs, references = self.decoder(
            ...
            input_query_keypoints=input_query_keypoints.transpose(0, 1) if input_query_keypoints is not None else None,
            level_start_index=level_start_index,
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios,
            tgt_mask=attn_mask,
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.register_buffer("select_index", torch.tensor(self.kpt_index))

    def forward(
        self,
        tgt,  # (1000, 8, 256)
        memory,
        tgt_mask: Optional[Tensor] = None,  # (64, 1000, 1000)
        tgt_mask2: Optional[Tensor] = None,  # (64, 3000, 3000)
        memory_mask: Optional[Tensor] = None,
        tgt_key_padding_mask: Optional[Tensor] = None,
        memory_key_padding_mask: Optional[Tensor] = None,
        pos: Optional[Tensor] = None,
        refpoints_unsigmoid: Optional[Tensor] = None,  # num_queries, bs, 4 (100 + 900, 8, 4)
        input_query_keypoints: Optional[Tensor] = None,  # num_queries, bs, 14 * 2 (100, 8, 28)
        # for memory
        ...
        intermediate = []
        reference_points = refpoints_unsigmoid.sigmoid()  # (1000,8,4)
        ...
        for layer_id, layer in enumerate(self.layers):
            if self.deformable_decoder:
                if reference_points.shape[-1] == 4:  # True
                    reference_points_input = (
                        reference_points[:, :, None]  # (100 + 900, 8, 1, 4)
            ...
            assert not output.isinf().any() and not output.isnan().any(), f"{layer_id=}, {output.shape=}, {output=}"
            assert not query_pos.isinf().any() and not query_pos.isnan().any(), (
                f"{layer_id=}, {query_pos.shape=}, {query_pos=}"
            )
            assert not query_sine_embed.isinf().any() and not query_sine_embed.isnan().any(), (
                f"{layer_id=}, {query_sine_embed.shape=}, {query_sine_embed=}"
            )
            assert not reference_points_input.isinf().any() and not reference_points_input.isnan().any(), (
                f"{layer_id=}, {reference_points_input.shape=}, {reference_points_input=}"
            )
            ...
            intermediate.append(self.norm(output))  # (1000/3000, bs, 256)

            # human update
            if layer_id < self.num_box_decoder_layers:
                reference_before_sigmoid = inverse_sigmoid(reference_points)
                delta_unsig = self.bbox_embed[layer_id](output)
                outputs_unsig_norm = delta_unsig + reference_before_sigmoid
                new_reference_points = outputs_unsig_norm.sigmoid()
            ...
            if layer_id == self.num_box_decoder_layers - 1:
                ...
                if input_query_keypoints is not None:
                    dn_output = output[:effect_num_dn]  # (100, bs, 256)
                    dn_reference_points = new_reference_points[:effect_num_dn]  # [100, bs, 4]

                    dn_newoutput = (
                        dn_output[:, None, :, :] + self.keypoint_embed.weight[None, :, None, :]
                    )  # (100, 14, bs, 256)

                    dn_newoutput = torch.cat([dn_output[:, None, :, :], dn_newoutput], dim=1).flatten(
                        0, 1
                    )  # (100 * (1 + num_joints), bs, 256)

                    dn_keypoint = input_query_keypoints.sigmoid()  # (100, bs, 14 * 2)
                    dn_keypoint = dn_keypoint.reshape(-1, bs, self.num_body_points, 2).transpose(
                        1, 2
                    )  # (100, 14, bs, 2)
                    keypoint_wh_weight = (
                        self.hw.weight.unsqueeze(0)  # (1, 14, 2)
                        .unsqueeze(-2)  # (1, 14, 1, 2)
                        .repeat(effect_num_dn, 1, bs, 1)  # (100, 14, bs, 2)
                        .sigmoid()
                    )

                    keypoint_wh = keypoint_wh_weight * dn_reference_points[..., 2:][:, None, :, :]  # (100, 14, bs, 2)
                    dn_keypoint = torch.cat(
                        [
                            dn_keypoint,  # (100, 14, bs, 2)
                            keypoint_wh,
                        ],
                        dim=-1,
                    )  # (100,14,bs,4)
                    dn_new_reference_points = torch.cat(
                        [
                            dn_reference_points[:, None, :, :],
                            dn_keypoint,
                        ],
                        dim=1,
                    ).flatten(0, 1)  # (100 * 15, 8, 4)
                # ==== End update DN ====
                ...
                new_output_for_keypoint = (
                    new_output_for_box[:, None, :, :]  # （100, 1, bs, 256)
                    + self.keypoint_embed.weight[None, :, None, :]  # (1, 14, 1, 256)
                )  # (100, 14, 8, 256)
                ...
                new_reference_points_for_keypoint_norm = torch.cat((keypoint_xy, keypoint_wh), dim=-1)
                new_reference_points = torch.cat(
                    (
                        new_reference_points_for_box.unsqueeze(1),
                        new_reference_points_for_keypoint_norm,
                    ),
                    dim=1,
                ).flatten(0, 1)
                ...
                if input_query_keypoints is not None:
                    output = torch.cat((dn_newoutput, output), dim=0)
                else:
                    output = output
                tgt_mask = tgt_mask2

                effect_num_dn = effect_num_dn * (1 + self.num_body_points)

            # =========================================================
            # after expansion (1600, 8, 256)
            # (100 + 100 * (1 + 14), bs, dim)-> (100dn + num_groups * (1+num_keypoints), bs, dim)

            # proposed: (3000, 8 ,256)
            # (100 * (1 + 14) + 100 * (1 + 14), bs, dim)
            # =========================================================
            ...
            if layer_id >= self.num_box_decoder_layers:
                ...
                output_kpt_norm = output[effect_num_dn:].index_select(0, self.select_index)
                delta_xy_unsig_norm = self.pose_embed[layer_id - self.num_box_decoder_layers](output_kpt_norm)
                outputs_unsig_norm = (
                    reference_before_sigmoid[effect_num_dn:].index_select(0, self.select_index).clone()
                )  ##
                delta_hw_unsig_norm = self.pose_hw_embed[layer_id - self.num_box_decoder_layers](output_kpt_norm)
                outputs_unsig_norm[..., :2] += delta_xy_unsig_norm[..., :2]
                outputs_unsig_norm[..., 2:] += delta_hw_unsig_norm
                new_reference_points_for_keypoint_norm = outputs_unsig_norm.sigmoid()

                new_reference_points_norm = torch.cat(
                    (
                        new_reference_points_for_box_norm.unsqueeze(1),
                        new_reference_points_for_keypoint_norm.view(-1, self.num_body_points, bs, 4),
                    ),
                    dim=1,
                ).flatten(0, 1)
                ...
                if input_query_keypoints is not None:
                    output_bbox_dn = output[:effect_num_dn][0 :: (self.num_body_points + 1)]
                    ...
                    output_kpt_dn = output[:effect_num_dn].index_select(0, self.select_index)
                    delta_xy_unsig_dn = self.pose_embed[layer_id - self.num_box_decoder_layers](output_kpt_dn)
                    outputs_unsig_dn = (
                        reference_before_sigmoid[:effect_num_dn].index_select(0, self.select_index).clone()
                    )  ##
                    delta_hw_unsig_dn = self.pose_hw_embed[layer_id - self.num_box_decoder_layers](output_kpt_dn)
                    outputs_unsig_dn[..., :2] += delta_xy_unsig_dn[..., :2]
                    outputs_unsig_dn[..., 2:] += delta_hw_unsig_dn

                    new_reference_points_for_keypoint_dn = outputs_unsig_dn.sigmoid()

                    new_reference_points_dn = torch.cat(
                        (
                            new_reference_points_for_box_dn.unsqueeze(1),
                            new_reference_points_for_keypoint_dn.view(-1, self.num_body_points, bs, 4),
                        ),
                        dim=1,
                    ).flatten(0, 1)
                    new_reference_points = torch.cat((new_reference_points_dn, new_reference_points_norm), dim=0)
