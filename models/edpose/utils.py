...
class SymmetricGatedFusion(nn.Module):
    ...
    def forward(self, sa_out, ca_out):
        cat = torch.cat([sa_out, ca_out], dim=-1)
        h = self.mlp(cat)  # (..., 2)
        sa_weight, ca_weight = h.split(1, dim=-1)  # (..., 1), (..., 1)
        sa_out = sa_out * sa_weight
        ca_out = ca_out * ca_weight
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x = F.gelu(layer(x), approximate="tanh") if i < self.num_layers - 1 else layer(x)
        return x
...
def oks_overlaps(kpt_preds, kpt_gts, kpt_valids, kpt_areas, sigmas):
    assert (kpt_areas > 0).all(), f"{kpt_areas=}"
    assert (sigmas > 0).all(), f"{sigmas=}"
...
def oks_loss(pred, target, valid=None, area=None, linear=False, sigmas=None, eps=1e-6):
    """Oks loss.
