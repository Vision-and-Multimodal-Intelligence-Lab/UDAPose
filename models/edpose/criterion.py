...
from .utils import OKSLoss, sigmoid_focal_loss
...
class SetCriterion(nn.Module):
    ...
    def loss_keypoints(self, outputs, targets, indices, num_boxes):
        ...
        if len(src_keypoints) == 0:
            ...
            losses = {
                "loss_keypoints": torch.as_tensor(0.0, device=device) + src_keypoints.sum() * 0,
                "loss_oks": torch.as_tensor(0.0, device=device) + src_keypoints.sum() * 0,
            }
            return losses
    ...
    def loss_dn_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if "num_tgt" not in outputs:
            device = outputs["pred_logits"].device
            losses = {
                "dn_loss_bbox": torch.as_tensor(0.0, device=device),
                "dn_loss_giou": torch.as_tensor(0.0, device=device),
            }
            return losses
    ...
    def loss_dn_keypoints(self, outputs, targets, indices, num_boxes):
        ...
        if "num_tgt" not in outputs:
            device = outputs["pred_logits"].device
            losses = {
                "dn_loss_keypoints": torch.as_tensor(0.0, device=device),
                "dn_loss_oks": torch.as_tensor(0.0, device=device),
            }
            return losses

        num_tgt = outputs["num_tgt"]
        src_keypoints = outputs["dn_keypoints_pred"]
        tgt_keypoints = outputs["dn_keypoints_input"]
        area = outputs["dn_areas_input"]

        return self.tgt_loss_keypoints(src_keypoints, tgt_keypoints, area, num_tgt)
    ...
    def loss_dn_labels(self, outputs, targets, indices, num_boxes):
        ...
        if "num_tgt" not in outputs:
            device = outputs["pred_logits"].device
            losses = {
                "dn_loss_ce": torch.as_tensor(0.0, device=device),
            }
            return losses
        ...
        if "aux_outputs" in outputs:
            for idx, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    if loss == "masks":
                        continue
                    if loss in ["keypoints", "dn_keypoints"] and idx < self.num_box_decoder_layers:
                        continue
                    kwargs = {}
                    if loss == "labels":
        ...
        if "query_expand" in outputs:
            ...
            for loss in self.losses:
                ...
                if loss == "labels":
                    kwargs = {"log": False}
                l_dict = self.get_loss(loss, interm_outputs, targets, indices, num_boxes, **kwargs)
                l_dict = {k + "_query_expand": v for k, v in l_dict.items()}
                losses.update(l_dict)
    ...
    def tgt_loss_keypoints(
        self,
        src_keypoints,  # (bs, 100, 42)
        tgt_keypoints,  # (bs, 100, 42)
        area,
        num_tgt,  # 100
    ):
        assert num_tgt == src_keypoints.shape[1] == tgt_keypoints.shape[1], (
            f"{num_tgt=}, {src_keypoints.shape=}, {tgt_keypoints.shape=}"
        )

        src_keypoints = src_keypoints.reshape(-1, 42)
        tgt_keypoints = tgt_keypoints.reshape(-1, 42)  # (bs*100, 42)
        Z_pred = src_keypoints[:, 0 : (self.num_body_points * 2)]
        V_pred = src_keypoints[:, (self.num_body_points * 2) :]

        Z_gt = tgt_keypoints[:, 0 : (self.num_body_points * 2)]  # (bs*100, 28)
        V_gt = tgt_keypoints[:, (self.num_body_points * 2) :]  # (bs*100, 14)

        assert (area > 0).all(), f"{area.min()=}, {area.max()=},{area=}"
        area = area.flatten()  # (bs, 100) -> (bs*100,)
        oks_loss = self.oks(
            Z_pred,
            Z_gt,
            V_gt,
        ...
        losses = {}
        losses["dn_loss_keypoints"] = pose_loss.sum() / num_tgt
        losses["dn_loss_oks"] = oks_loss.sum() / num_tgt
        return losses
