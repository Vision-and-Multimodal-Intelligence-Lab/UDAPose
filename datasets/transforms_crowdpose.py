...
def crop(image, target, region):
    ...
    if "keypoints" in target:
        ...
        cropped_keypoints = torch.cat(
            [cropped_keypoints, keypoints.view(-1, 3)[:, 2].unsqueeze(1)], dim=1
        )
        target["keypoints"] = cropped_keypoints.view(
            target["keypoints"].shape[0], 14, 3
        )
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "keypoints" in target:
            ...
            V = keypoints[
                :, :, 2
            ]  # visibility of the keypoints torch.Size([number of persons, 14])
            V[V == 2] = 1
            Z = keypoints[:, :, :2]
            ...
            all_keypoints = torch.cat(
                [Z, V], dim=1
            )  # torch.Size([number of persons, 2+28+14])
